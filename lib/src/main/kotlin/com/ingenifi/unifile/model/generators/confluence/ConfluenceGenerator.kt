...
data class ConfluenceGenerator(val config: SectionGeneratorConfig, val number: Int, val file: File, val headingName: HeadingName = HEADING_NAME) : SectionsGenerator,
    ...
    override fun generate(): Sections {
        ...
        val list = file.readLines().map(::createLink).map {
            createSection(it, withLevel)
        }
        return Sections(list = list, numberProcessed = numberProcessed)
    }
    ...
    private fun createSection(link: ConfluenceLink, withLevel: Int): Section {
        ...
        val section = Section(
            heading = Heading(
                headingName = headingName, sectionNumber = SectionNumber(listOf(Level(number + numberProcessed))), title = TitleText.Title(link.title)
